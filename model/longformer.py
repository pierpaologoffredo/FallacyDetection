...
class newLongformerForTokenClassification(LongformerPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]

    def __init__(self, config, config_feat, alpha, config_pos = None):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        self.longformer_feat = LongformerModel(config=config_feat)
        
        if config_pos is not None:
            self.num_pos = config_pos.num_labels
            self.longformer_pos = LongformerModel(config=config_pos)
            self.pos_classifier = nn.Linear(config_pos.hidden_size, config_pos.num_labels)
    ...
    def forward(
        ...
        if arg_comps is not None:
            comp_outputs = self.longformer_feat(
                input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
        ...
        if arg_rels is not None:
            rel_outputs = self.longformer_feat(
                input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
        ...
        if pos_tags is not None:
            pos_outputs = self.longformer_pos(
                input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
            ...
            pos_sequence_output = self.dropout(pos_sequence_output)
            pos_logits = self.pos_classifier(pos_sequence_output)
        ...
        if labels is not None:
            ...
            labels = labels.to(logits.device)
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            if arg_comps is not None and arg_rels is None and pos_tags is None:
            ...
            elif arg_rels is not None and arg_comps is None and pos_tags is None:
            ...
            elif arg_comps is not None and arg_rels is not None and pos_tags is None:
                arg_rels = arg_rels.to(rel_logits.device)
                arg_comps = arg_comps.to(comp_logits.device)
                comp_loss = loss_fct(comp_logits.view(-1, self.num_comp), arg_comps.view(-1))
                rel_loss = loss_fct(rel_logits.view(-1, self.num_comp), arg_rels.view(-1))
                loss = self.alpha * ((comp_loss + rel_loss + loss))/3
            elif pos_tags is not None and arg_rels is None and arg_comps is None:
                pos_tags = pos_tags.to(pos_logits.device)
                pos_loss = loss_fct(pos_logits.view(-1, self.num_pos), pos_tags.view(-1))
                loss = self.alpha * ((pos_loss + loss))/2
            elif pos_tags is not None and arg_rels is not None and arg_comps is None:
                pos_tags = pos_tags.to(pos_logits.device)
                pos_loss = loss_fct(pos_logits.view(-1, self.num_pos), pos_tags.view(-1))
                arg_rels = arg_rels.to(rel_logits.device)
                rel_loss = loss_fct(rel_logits.view(-1, self.num_comp), arg_rels.view(-1))
                loss = self.alpha * ((pos_loss + rel_loss + loss))/3
            elif pos_tags is not None and arg_rels is None and arg_comps is not None:
                pos_tags = pos_tags.to(pos_logits.device)
                pos_loss = loss_fct(pos_logits.view(-1, self.num_pos), pos_tags.view(-1))
                arg_comps = arg_comps.to(comp_logits.device)
                comp_loss = loss_fct(comp_logits.view(-1, self.num_comp), arg_comps.view(-1))
                loss = self.alpha * ((comp_loss + pos_loss + loss))/3
            elif pos_tags is not None and arg_rels is not None and arg_comps is not None:
                pos_tags = pos_tags.to(pos_logits.device)
                pos_loss = loss_fct(pos_logits.view(-1, self.num_pos), pos_tags.view(-1))
                arg_rels = arg_rels.to(rel_logits.device)
                arg_comps = arg_comps.to(comp_logits.device)
                comp_loss = loss_fct(comp_logits.view(-1, self.num_comp), arg_comps.view(-1))
                rel_loss = loss_fct(rel_logits.view(-1, self.num_comp), arg_rels.view(-1))
                loss = self.alpha * ((comp_loss + rel_loss + pos_loss + loss))/4
