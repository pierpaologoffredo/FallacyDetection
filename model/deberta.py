...
class newDebertaForTokenClassification(DebertaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config, config_feat, alpha, config_pos = None):
        ...
        self.deberta_feat = DebertaModel(config=config_feat)
        
        if config_pos is not None:
            self.num_pos = config_pos.num_labels
            self.deberta_pos = DebertaModel(config=config_pos)
    ...
    def forward(
        ...
        if arg_comps is not None:
            comp_outputs = self.deberta_feat(
                input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
